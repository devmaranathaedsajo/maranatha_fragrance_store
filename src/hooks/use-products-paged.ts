 ...
 export type UseProductsPagedParams = {
     page: number;
     pageSize: number;
     query?: string;
 };
 
 export type UseProductsPagedResult = {
     products: Product[];
     total: number;
     totalPages: number;
     isLoading: boolean;
 ...
 export function useProductsPaged(
     params: UseProductsPagedParams,
 ): UseProductsPagedResult {
     ...
     const fetchPage = useCallback(async () => {
         ...
         if (query) {
             ...
             q = q.or(
                 `name.ilike.%${escaped}%,sku.ilike.%${escaped}%,slug.ilike.%${escaped}%`,
             );
         }
