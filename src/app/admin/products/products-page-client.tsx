...
import { useProductsPaged } from "@/hooks/use-products-paged";
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${
        active
          ? "border-[color-mix(in_srgb,var(--brand-primary)_18%,transparent)] bg-[color-mix(in_srgb,var(--brand-primary)_8%,transparent)] text-(--brand-primary)"
          : "border-[color-mix(in_srgb,var(--foreground)_14%,transparent)] bg-[color-mix(in_srgb,var(--foreground)_5%,transparent)] text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]"
      }`}
    >
      {label}
    </span>
...
function TableRow({
  ...
  return (
    <tr className="border-b border-[color-mix(in_srgb,var(--foreground)_10%,transparent)]">
      ...
      <td className="whitespace-nowrap px-3 py-3 text-sm text-[color-mix(in_srgb,var(--foreground)_75%,transparent)]">
        {product.sku}
      </td>
      <td className="whitespace-nowrap px-3 py-3">
...
export default function ProductsPageClient({
  ...
  const { products, total, totalPages, isLoading, error } = useProductsPaged({
    page,
    pageSize,
    query: q,
  });
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() =>
                pushParams({ q: searchDraft.trim() || null, page: 1 })
              }
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {isLoading ? (
                ...
                products.map((p) => (
                  <TableRow key={p.id} product={p} actions={actions} />
                ))
              )}
            </tbody>
          </table>
