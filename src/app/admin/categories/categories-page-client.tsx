...
import { useCategoriesPaged } from "@/hooks/use-categories-paged";
  ...
  return (
    <tr className="border-b border-[color-mix(in_srgb,var(--foreground)_10%,transparent)]">
      ...
      <td className="whitespace-nowrap px-3 py-3 text-sm text-[color-mix(in_srgb,var(--foreground)_75%,transparent)]">
        {category.parent_id ? (
          <span className="font-mono text-xs">{category.parent_id}</span>
        ) : (
      ...
      <td className="whitespace-nowrap px-3 py-3 text-xs text-[color-mix(in_srgb,var(--foreground)_55%,transparent)]">
        {new Date(category.created_at).toLocaleDateString("es-DO")}
      </td>
      <td className="whitespace-nowrap px-3 py-3">
        {actions.length ? (
...
export default function CategoriesPageClient({
  ...
  const { categories, total, totalPages, isLoading, error } = useCategoriesPaged({
    page,
    pageSize,
    query: q,
  });
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {isLoading ? (
                ...
                categories.map((c) => (
                  <TableRow key={c.id} category={c} actions={actions} />
                ))
              )}
            </tbody>
          </table>
